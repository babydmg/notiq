...
router.post("/", auth, async (req, res) => {
  ...
  try {
    ...
    const recurringJob = result.rows[0];
    ...
    const job = await emailQueue.add(
      ...
      {
        repeat: { pattern: cron },
        jobId: `recurring_${recurringJob.id}`,
      },
    );

    await pool.query(
      `UPDATE recurring_jobs SET bull_job_id = $1 WHERE id = $2`,
      [job.id, recurringJob.id],
    );
...
router.delete("/:id", auth, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT * FROM recurring_jobs WHERE id = $1 AND tenant_id = $2`,
    ...
    // Remove from BullMQ
    await emailQueue.removeRepeatableByKey(`recurring_${job.id}`);

    // Mark as inactive in DB
    await pool.query(`UPDATE recurring_jobs SET active = false WHERE id = $1`, [
      job.id,
    ]);
